...
static esp8266at_err_t _send_cmd_and_wait_rsp(esp8266at_t *esp8266at, char *cmd, char *rsp, uint32_t timeoutms);
...
static esp8266at_err_t _wait_rsp(esp8266at_t *esp8266at, char *rsp, uint8_t *buffer, uint32_t length, uint32_t *received, uint32_t timeoutms) {
	...
	do {
		...
		while ((rsp_i < rsp_len) && (buf_i < length - 1)) {
			err = esp8266at_io_read_timedms(esp8266at, &buffer[buf_i], 1, &read, timeoutms);
...
static esp8266at_err_t _send_cmd_and_wait_rsp(esp8266at_t *esp8266at, char *cmd, char *rsp, uint32_t timeoutms) {
	...
	do {
		err = esp8266at_io_read_clear_timedms(esp8266at, timeoutms);
		timeoutms = task_getremainingtimeoutms();
		if (err != ESP8266AT_OK) {
			break;
		}

		err = esp8266at_io_write_timedms(esp8266at, (uint8_t*) cmd, strlen(cmd), NULL, timeoutms);
		timeoutms = task_getremainingtimeoutms();
		if (err != ESP8266AT_OK) {
			break;
		}

		err = _wait_rsp(esp8266at, rsp, esp8266at->rsp_buf, ESP8266AT_RSP_BUFFER_SIZE, NULL, timeoutms);
		timeoutms = task_getremainingtimeoutms();
		if (err != ESP8266AT_OK) {
			break;
		}
...
esp8266at_err_t esp8266at_cmd_at_cipsend(esp8266at_t *esp8266at, uint8_t *buffer, uint32_t length, uint32_t timeoutms) {
	...
	do {
		...
		err = esp8266at_io_write_timedms(esp8266at, buffer, length, NULL, timeoutms);
		timeoutms = task_getremainingtimeoutms();
		if (err != ESP8266AT_OK) {
			break;
		}

		err = _wait_rsp(esp8266at, "SEND OK\r\n", esp8266at->rsp_buf, ESP8266AT_RSP_BUFFER_SIZE, NULL, timeoutms);
		timeoutms = task_getremainingtimeoutms();
		if (err != ESP8266AT_OK) {
			break;
		}
...
esp8266at_err_t esp8266at_cmd_at_ciprecv(esp8266at_t *esp8266at, uint8_t *buffer, uint32_t length, uint32_t *received, uint32_t timeoutms) {
	...
	do {
		...
		err = _wait_rsp(esp8266at, "+IPD,", esp8266at->rsp_buf, ESP8266AT_RSP_BUFFER_SIZE, NULL, timeoutms);
		timeoutms = task_getremainingtimeoutms();
		if (err != ESP8266AT_OK) {
			break;
		}

		err = _wait_rsp(esp8266at, ":", esp8266at->rsp_buf, ESP8266AT_RSP_BUFFER_SIZE, &read, timeoutms);
		timeoutms = task_getremainingtimeoutms();
		if (err != ESP8266AT_OK) {
			break;
		}
		...
		err = esp8266at_io_read_timedms(esp8266at, buffer, data_len, &read, timeoutms);
		timeoutms = task_getremainingtimeoutms();
