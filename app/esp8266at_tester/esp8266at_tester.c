...
static void cli_at_test()
{
    ...
    err = esp8266at_cmd_at_test(&_g_esp8266at, _timeoutms, NULL);
    printf("result : err = %d\n", err);
}
...
static void cli_at_restart()
{
    ...
    err = esp8266at_cmd_at_rst(&_g_esp8266at, _timeoutms, NULL);
    printf("result : err = %d\n", err);
}
...
static void cli_at_query_version()
{
    ...
    err = esp8266at_cmd_at_gmr(&_g_esp8266at, _timeoutms, NULL);
    printf("result : err = %d, version = %s\n", err, _g_esp8266at.version);
}
...
static int cli_at_config_echo(char *str, int len, void *arg)
{
    ...
    do
    {
        ...
        err = esp8266at_cmd_at_e(&_g_esp8266at, is_on, _timeoutms, NULL);
        printf("result : err = %d\n", err);
        r = 0;
...
static int cli_at_config_wmode(char *str, int len, void *arg)
{
    ...
    err = esp8266at_cmd_at_cwmode(&_g_esp8266at, mode, _timeoutms, NULL);
    printf("result : err = %d\n", err);
    r = 0;
...
static int cli_at_config_ipmux(char *str, int len, void *arg)
{
    ...
    err = esp8266at_cmd_at_cipmux(&_g_esp8266at, mode, _timeoutms, NULL);
    printf("result : err = %d\n", err);
    r = 0;
...
static void cli_at_ap_quit(void)
{
    ...
    err = esp8266at_cmd_at_cwqap(&_g_esp8266at, _timeoutms, NULL);
    printf("result : err = %d\n", err);
}
...
static void cli_at_query_ip(void)
{
    ...
    err = esp8266at_cmd_at_cifsr(&_g_esp8266at, _timeoutms, NULL);
    printf("result : err = %d, ip_addr = %s, mac_addr = %s\n", err, _g_esp8266at.ip_addr, _g_esp8266at.mac_addr);
}
...
static int cli_at_conn_open(char *str, int len, void *arg)
{
    ...
    do
    {
        ...
        err = esp8266at_cmd_at_cipstart(&_g_esp8266at, type, ip, port, _timeoutms, NULL);
        printf("result : err = %d\n", err);
        r = 0;
...
static void cli_at_conn_close(void)
{
    ...
    err = esp8266at_cmd_at_cipclose(&_g_esp8266at, _timeoutms, NULL);
    printf("result : err = %d\n", err);
}
...
static int cli_at_conn_send(char *str, int len, void *arg)
{
    ...
    err = esp8266at_cmd_at_cipsend(&_g_esp8266at, (uint8_t*) str, (uint32_t) len, _timeoutms, NULL);
    printf("result : err = %d\n", err);
...
static void cli_at_conn_recv(void)
{
    ...
    err = esp8266at_cmd_at_ciprecv(&_g_esp8266at, _recv_buf, ESP8266AT_RECV_BUFFER_SIZE, &read, _timeoutms, NULL);
    _recv_buf[read] = 0;
...
static int cli_echo_client(char *str, int len, void *arg)
{
    ...
    do
    {
        ...
        printf("\n==== Quit from the AP ====\n\n");
        esp8266at_cmd_at_cwqap(&_g_esp8266at, _timeoutms, NULL);
        ...
        printf("\n==== Config echo on ====\n\n");
        err = esp8266at_cmd_at_e(&_g_esp8266at, 1, _timeoutms, NULL);
        if (err != ESP8266AT_ERR_OK)
        {
            r = -1;
            break;
        ...
        err = esp8266at_cmd_at_cipmux(&_g_esp8266at, 0, _timeoutms, NULL);
        if (err != ESP8266AT_ERR_OK)
        {
            r = -1;
            break;
        ...
        printf("\n==== Config WiFi mode 1 ====\n\n");
        err = esp8266at_cmd_at_cwmode(&_g_esp8266at, 1, _timeoutms, NULL);
        if (err != ESP8266AT_ERR_OK)
        {
            r = -1;
            break;
        ...
        printf("\n==== Open connection ====\n\n");
        err = esp8266at_cmd_at_cipstart(&_g_esp8266at, "TCP", ip, port, _timeoutms, NULL);
        if (err != ESP8266AT_ERR_OK)
        {
            r = -1;
            break;
        ...
        for (uint32_t i = 0; i < count; i++)
        {
            ...
            msglen = strlen(msg);
            err = esp8266at_cmd_at_cipsend(&_g_esp8266at, (uint8_t*) msg, msglen, _timeoutms, NULL);
            if (err != ESP8266AT_ERR_OK)
            {
                break;
            }
    ...
    {
        read = 0;
        esp8266at_io_read_timedms(&_g_esp8266at, _recv_buf, ESP8266AT_RECV_BUFFER_SIZE, &read, _timeoutms, NULL);
        _recv_buf[read] = 0;
        printf("\"%s\"\n", (char*) _recv_buf);
    }
